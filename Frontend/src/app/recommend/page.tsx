...
import { getUserRecommend } from '@/api/auth';
...
export default function Home() {
  ...
  useEffect(() => {
    const getMovies = async () => {
      try {
        setIsLoading(true);
        const response = await getUserRecommend();
        setMovies(response);
        setTotalPages(1);
      } catch (error) {
  ...
  return (
    <div className="pt-16 min-h-screen bg-gradient-to-b from-gray-900 via-black to-gray-900 text-white">
      <div className="container mx-auto py-8">
        <h1 className="text-4xl font-extrabold mb-6 text-center">Recommendation</h1>
              ...
              {movies.map((movie) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
                    ...
                    <p className="text-sm mb-2">{movie.overview.slice(0, 100)}...</p>
                    <p className="text-xs">
